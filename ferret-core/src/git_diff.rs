...
impl GitChangeDetector {
    ...
    /// wins. Paths under `.ferret_index/` are always excluded since those are
    ...
    pub fn detect_changes(&self) -> Result<Vec<ChangeEvent>> {
        ...
        let mut events: Vec<ChangeEvent> = changes
            .into_iter()
            .filter(|(path, _)| !is_ferret_index_path(path))
...
/// Check whether a path is under the `.ferret_index/` directory.
///
/// These are index files (segments, tombstones, etc.) and should never be
/// reported as source-file changes.
fn is_ferret_index_path(path: &Path) -> bool {
    path.starts_with(".ferret_index")
}
...
mod tests {
    ...
    fn test_get_head_sha_in_real_repo() {
        // This test runs inside the ferret repository which is a git repo.
    ...
    // ---- .ferret_index path filtering ------------------------------------

    #[test]
    fn test_is_ferret_index_path() {
        assert!(is_ferret_index_path(&PathBuf::from(
            ".ferret_index/segments/seg_0000/trigrams.bin"
        )));
        assert!(is_ferret_index_path(&PathBuf::from(".ferret_index/lock")));
        assert!(is_ferret_index_path(&PathBuf::from(".ferret_index")));
        assert!(!is_ferret_index_path(&PathBuf::from("src/main.rs")));
        assert!(!is_ferret_index_path(&PathBuf::from(".gitignore")));
        assert!(!is_ferret_index_path(&PathBuf::from(
            "foo/.ferret_index/bar"
        )));
    }

    #[test]
    fn test_parse_untracked_nul_filters_ferret_index() {
        // Simulate git reporting .ferret_index files as untracked (NUL-delimited)
        let output = "src/main.rs\0.ferret_index/segments/seg_0000/trigrams.bin\0.ferret_index/lock\0lib.rs\0";
        let events = parse_untracked_nul(output);

        // parse_untracked_nul itself doesn't filter — the filter is in detect_changes.
        // But we can test is_ferret_index_path on the results.
        let filtered: Vec<_> = events
            .into_iter()
            .filter(|e| !is_ferret_index_path(&e.path))
            .collect();
