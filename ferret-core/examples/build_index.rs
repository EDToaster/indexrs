...
//!
//! If `.ferret_index/` already exists under the target directory, detects changes
...
//!
//! Usage:
//!   cargo run -p ferret-indexer-core --example build_index -- <directory>
//!   cargo run -p ferret-indexer-core --example build_index --release -- <directory>
//!
//! Examples:
//!   cargo run -p ferret-indexer-core --example build_index --release -- .
//!   cargo run -p ferret-indexer-core --example build_index --release -- ~/src/my-repo
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        ...
        eprintln!();
        eprintln!("Builds or updates an ferret index, then prints stats.");
        eprintln!("Index is stored in <directory>/.ferret_index/");
        std::process::exit(1);
    }
    let dir = PathBuf::from(&args[1]);
    let index_dir = dir.join(".ferret_index");
