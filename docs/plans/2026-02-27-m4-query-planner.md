# Query Planner Implementation Plan (HHC-48)

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Implement a query planner that converts a Query AST (from HHC-46) into an optimized execution plan. The planner selects which index to consult for each query term, applies pre-filters (language, path) first to produce a cheap candidate bitmap, orders trigram posting list intersections by estimated size (smallest first) using the `file_list_len` field from the trigram table header, and structures the plan as: `Filter(lang) -> Filter(path) -> Intersect(trigrams) -> Verify(content)`.
...
**Dependencies:**
- ASCII case-fold trigrams plan -- the index stores lowercase trigrams
- HHC-46 (Query AST) -- provides the `Query` struct with filters and pattern
- HHC-47 (Trigram extraction from patterns) -- provides `extract_query_trigrams()` function
...
**Tests:** 4 unit tests for type construction, empty plans, pre-filters, and regex verification.

**Exit criteria:** All 4 tests pass. `QueryPlan`, `PreFilter`, `VerifyStep`, and `ScoredTrigram` types compile and are exported.
...
### Step 2: Implement `estimate_posting_list_size`

Add to `TrigramIndexReader`:
...
**Tests:** 4 unit tests covering shared trigrams, single-file trigrams, absent trigrams, and empty index.

**Exit criteria:** All 4 tests pass. `estimate_posting_list_size()` returns the correct `file_list_len` without decoding posting lists.
...
### Step 2: Implement `plan_literal_query()`

Add to `query_plan.rs`:
...
**Tests:** 5 unit tests for literal planning, language filter, short query, absent trigrams, and path filter.

**Exit criteria:** All tests pass. `plan_literal_query()` produces correctly-ordered trigram plans.
...
fn test_plan_regex_query() {
    ...
    // but we can still extract trigrams from longer required substrings if present
...
### Step 2: Implement `plan_regex_query()`
...
**Tests:** 3 new tests for regex planning, no-extractable-trigrams case, and regex with filters.

**Exit criteria:** All tests pass. `plan_regex_query()` extracts literal runs from regex patterns and produces correctly-ordered trigram plans. `extract_literal_runs()` handles escapes and metacharacters.
...
### Step 2: Implement `plan_query()` and `plan_query_multi()`
...
pub fn plan_query(input: &QueryInput, segment: &Segment) -> QueryPlan {
    match input {
        QueryInput::Literal { pattern, filters } => {
            plan_literal_query(pattern, filters, segment)
        }
        QueryInput::Regex { pattern, filters } => {
            plan_regex_query(pattern, filters, segment)
        }
    }
}
...
**Tests:** 3 new tests for unified literal planning, regex planning, and multi-segment planning.

**Exit criteria:** All tests pass. `plan_query()` dispatches correctly. `plan_query_multi()` produces per-segment plans.
...
### Step 2: Add `can_short_circuit()` to `QueryPlan`
...
**Tests:** 4 new tests for zero-count ordering, deduplication, short-circuit detection, and non-short-circuit detection.

**Exit criteria:** All tests pass. `can_short_circuit()` correctly identifies plans that will produce empty results.
...
**Tests:** 2 new tests for Display formatting.

**Exit criteria:** All tests pass. All query plan types have useful Display implementations for debugging and logging.
...
This test builds a segment, creates a plan, and verifies that the plan's predictions (which trigrams, which order) align with actual search results.
...
### Step 2: No new production code needed

These tests validate existing functionality. If any test fails, the issue is in the planner logic from previous tasks.
...
**Tests:** 3 integration tests validating plan correctness against actual data.

**Exit criteria:** All tests pass. The planner's size estimates and ordering align with actual search behavior.
...
|------|-------------|-------|-------|
| 1 | Define `QueryPlan`, `PreFilter`, `VerifyStep`, `ScoredTrigram` types | 4 | query_plan.rs (create), lib.rs |
| 2 | Add `estimate_posting_list_size()` to `TrigramIndexReader` | 4 | index_reader.rs |
| 3 | Implement `plan_literal_query()` | 5 | query_plan.rs |
| 4 | Implement `plan_regex_query()` with literal run extraction | 3 | query_plan.rs |
| 5 | Implement `plan_query()` unified entry + `plan_query_multi()` | 3 | query_plan.rs |
| 6 | Add `can_short_circuit()` optimization | 4 | query_plan.rs |
| 7 | Add `Display` implementations | 2 | query_plan.rs |
| 8 | Integration tests: plan correctness vs actual search | 3 | query_plan.rs |

**Total: 8 tasks, 28 tests**

### Design Decisions

1. **Segment-specific plans:** Posting list sizes vary per segment, so each segment gets its own plan. This avoids the pitfall of using global statistics that may not reflect a specific segment's data distribution.

2. **Pre-filter before intersection:** Language and path filters are cheap (metadata scan) and can dramatically reduce the candidate set before the more expensive trigram posting list intersection. The plan explicitly orders pre-filters first.

3. **Smallest-first intersection ordering:** The existing `intersect_file_ids()` in `intersection.rs` already sorts by actual list length at execution time. The planner adds a new dimension: it uses estimated sizes (from the trigram table header, without decoding) to order the plan, allowing the executor to skip decoding large posting lists entirely if an earlier, smaller list produces few candidates.

4. **`estimate_posting_list_size()` vs `lookup_file_ids()`:** The planner uses `estimate_posting_list_size()` which reads the `file_list_len` field directly from the trigram table entry via O(log n) binary search. This avoids the varint decoding cost of `lookup_file_ids()`, which matters when comparing many trigrams' selectivity during planning.

5. **Bridge `QueryInput` type:** Until HHC-46's Query AST is available, the planner uses a `QueryInput` enum. This will be replaced with `&Query` once the parser is merged. The change is minimal: extract `pattern`, `filters`, and `query_type` from the AST and pass to the existing planning functions.

6. **Regex literal extraction:** The planner extracts required literal runs from regex patterns to generate trigrams for candidate filtering. This is a simple heuristic that will be enhanced by HHC-47's full regex trigram extraction. The current implementation handles escaped metacharacters and common patterns.

### Integration with Adjacent Issues

- **HHC-46 (Parser):** The planner accepts a `QueryInput` (bridge type) that will be replaced by the parser's `Query` AST. The parser extracts filters (`lang:rust`, `path:src/`) and the pattern (literal or regex).
- **HHC-47 (Trigram Extraction):** The planner calls `extract_unique_trigrams()` for literal queries and `extract_literal_runs()` + `extract_unique_trigrams()` for regex queries. Once HHC-47 provides `extract_query_trigrams()`, the regex path will use it.
- **HHC-49 (Verification):** The planner produces a `VerifyStep` enum that tells the verifier how to check candidates. The verifier receives file content and the `VerifyStep`, returning `Vec<LineMatch>`.
- **HHC-50 (Formatting):** The planner's output feeds into `multi_search.rs`, which produces `SearchResult` consumed by the formatter.
- **HHC-51 (Ranking):** The planner doesn't affect ranking directly, but the `ScoredTrigram` data could be used by the ranker to estimate query selectivity.
