...
/// Find the daemon binary (`indexrs`) by checking:
/// 1. Sibling of the current executable
/// 2. `INDEXRS_BIN` environment variable
/// 3. `indexrs` in `PATH` via `which`
pub fn find_daemon_binary() -> Result<PathBuf, IndexError> {
    // 1. Sibling of current exe (e.g. if MCP server is in the same dir as indexrs).
    if let Ok(current) = std::env::current_exe()
        && let Some(dir) = current.parent()
    {
        let sibling = dir.join("indexrs");
        if sibling.exists() {
            return Ok(sibling);
        }
    }
    ...
    if let Ok(val) = std::env::var("INDEXRS_BIN") {
    ...
    if let Ok(output) = std::process::Command::new("which").arg("indexrs").output()
    ...
    Err(IndexError::Io(std::io::Error::new(
        ...
        "could not find indexrs binary: not a sibling of current exe, \
         INDEXRS_BIN not set, and not found in PATH",
    )))
}
