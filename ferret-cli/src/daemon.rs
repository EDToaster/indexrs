...
use ferret_indexer_core::search::MatchPattern;
use ferret_indexer_core::types::FileId;

use ferret_indexer_daemon::json_protocol::{
    FileResponse, HealthResponse, JsonSearchFrame, JsonSymbolsFrame, SearchStats, StatusResponse,
    SymbolsStats,
};
pub use ferret_indexer_daemon::{DaemonRequest, DaemonResponse};
...
///
/// The daemon loads the index from `repo_root/.ferret_index/`, listens on the Unix
...
pub async fn start_daemon(repo_root: &Path, skip_catchup: bool) -> Result<(), IndexError> {
    ...
    // Fast-fail: must be initialized (has .ferret_index/).
    let ferret_dir = repo_root.join(".ferret_index");
    if !ferret_dir.is_dir() {
        return Err(IndexError::Config(format!(
            "repository not initialized (no .ferret_index/ directory): {} — run `ferret init` first",
            repo_root.display()
        )));
    }

    let sock_path = ferret_indexer_daemon::socket_path(repo_root);
...
fn handle_json_symbols_request(
    ...
    (
        Vec<ferret_indexer_core::symbol_index::SymbolMatch>,
        std::time::Duration,
    ),
    String,
> {
...
fn handle_json_symbols_request(
    ...
    (
        Vec<ferret_indexer_daemon::SymbolMatchResponse>,
        std::time::Duration,
    ),
    String,
> {
    ...
    while reader.read_line(&mut line).await.map_err(IndexError::Io)? > 0 {
        ...
        match request {
            ...
            DaemonRequest::QuerySearch {
                ...
                let parsed_query = match ferret_indexer_core::query::parse_query(&query) {
                    Ok(q) => q,
                    Err(e) => {
                        wire::write_response(
                            &mut writer,
            ...
            DaemonRequest::JsonSearch {
                ...
                let parsed_query = match ferret_indexer_core::query::parse_query(&full_query) {
                    Ok(q) => q,
                    Err(e) => {
                        wire::write_response(
                            &mut writer,
                ...
                let search_result = match tokio::task::spawn_blocking(move || {
                    ferret_indexer_core::multi_search::search_segments_with_query(
                        &snapshot,
                        &parsed_query,
                        &search_opts,
                    )
            ...
            DaemonRequest::Status => {
                ...
                if let Ok(entries) = std::fs::read_dir(&segments_dir) {
                    for entry in entries.filter_map(|e| e.ok()) {
                        ...
                        let dir = entry.path();
                        let dir_bytes = ferret_indexer_core::dir_size(&dir);
            ...
            DaemonRequest::Reindex { compact } => {
                ...
                let handle = tokio::task::spawn_blocking(move || {
                    ferret_indexer_core::run_catchup_with_progress(
                        &repo,
                        &idir,
                        &mgr,
                        compact,
...
mod tests {
    ...
    use ferret_indexer_daemon::try_connect;
    ...
    fn test_socket_path() {
        ...
        let path = ferret_indexer_daemon::socket_path(&root);
    ...
    async fn test_daemon_json_search_request() {
        use ferret_indexer_core::segment::InputFile;
        use ferret_indexer_daemon::json_protocol::JsonSearchFrame;
    ...
    async fn test_daemon_status_request() {
        use ferret_indexer_core::segment::InputFile;
        use ferret_indexer_daemon::json_protocol::StatusResponse;
    ...
    async fn test_daemon_health_request() {
        use ferret_indexer_daemon::json_protocol::HealthResponse;
    ...
    async fn test_daemon_get_file_request() {
        use ferret_indexer_core::segment::InputFile;
        use ferret_indexer_daemon::json_protocol::FileResponse;
