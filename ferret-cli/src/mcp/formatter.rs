...
use ferret_indexer_core::search::{FileMatch, LineMatch, SearchResult};
use ferret_indexer_core::types::Language;
...
pub fn format_index_status(status: &IndexStatusInfo) -> String {
    ...
    writeln!(out, "ferret status: healthy").unwrap();
    writeln!(out).unwrap();
...
pub fn format_symbol_results(
    query: &str,
    matches: &[ferret_indexer_core::symbol_index::SymbolMatch],
) -> String {
    let mut out = String::new();
...
mod tests {
    ...
    use ferret_indexer_core::search::{ContextLine, FileMatch, LineMatch, SearchResult};
    use ferret_indexer_core::types::{FileId, Language};
    ...
    fn test_format_index_status() {
        ...
        assert!(output.contains("ferret status: healthy"));
