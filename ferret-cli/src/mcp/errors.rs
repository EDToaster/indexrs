...
///
/// ```ignore
/// use ferret_mcp::errors::repo_not_found;
///
...
mod tests {
    ...
    fn test_repo_not_found_with_repos() {
        ...
        assert!(text.contains("ferret"), "should list available repos");
    ...
    fn test_repo_not_found_single_repo() {
        let result = repo_not_found("bar", &["ferret".into()]);
        assert_eq!(result.is_error, Some(true));
        let text = extract_text(&result);
        assert!(text.contains("Indexed repositories: ferret"));
    }
