//! MCP resource and resource template handlers for ferret.
//!
//! Exposes indexed data as MCP resources via `ferret://` URIs:
//!
//! - `ferret://status` -- overall index status
//! - `ferret://repo/{repo}/file/{+path}` -- file contents from the index
//! - `ferret://repo/{repo}/tree` -- directory tree listing
//! - `ferret://repo/{repo}/status` -- per-repo detailed status
...
use ferret_indexer_core::{IndexState, Language, Segment};
...
pub fn list_resources() -> ListResourcesResult {
    ListResourcesResult {
        resources: vec![
            RawResource {
                uri: "ferret://status".to_string(),
                name: "Index Status".to_string(),
                description: Some("Overall ferret index status".to_string()),
...
pub fn list_resource_templates() -> ListResourceTemplatesResult {
    ListResourceTemplatesResult {
        resource_templates: vec![
            RawResourceTemplate {
                uri_template: "ferret://repo/{repo}/file/{+path}".to_string(),
            ...
            .no_annotation(),
            RawResourceTemplate {
                uri_template: "ferret://repo/{repo}/tree".to_string(),
            ...
            .no_annotation(),
            RawResourceTemplate {
                uri_template: "ferret://repo/{repo}/status".to_string(),
...
fn read_status(state: &Arc<IndexState>) -> String {
    ...
    out.push_str("ferret status: healthy\n\n");
...
mod tests {
    ...
    fn test_parse_status() {
        assert_eq!(parse_uri("ferret://status"), Some(ResourceUri::Status));
    }

    #[test]
    fn test_parse_repo_file() {
        assert_eq!(
            parse_uri("ferret://repo/myproject/file/src/main.rs"),
    ...
    fn test_parse_repo_file_nested() {
        assert_eq!(
            parse_uri("ferret://repo/myproject/file/src/deep/nested/file.rs"),
            Some(ResourceUri::RepoFile {
                repo: "myproject".to_string(),
    ...
    fn test_parse_repo_tree() {
        assert_eq!(
            parse_uri("ferret://repo/myproject/tree"),
    ...
    fn test_parse_repo_status() {
        assert_eq!(
            parse_uri("ferret://repo/myproject/status"),
    ...
    fn test_parse_unknown_resource() {
        assert_eq!(parse_uri("ferret://unknown"), None);
    }

    #[test]
    fn test_parse_repo_no_action() {
        assert_eq!(parse_uri("ferret://repo/myproject"), None);
    }

    #[test]
    fn test_parse_repo_file_empty_path() {
        assert_eq!(parse_uri("ferret://repo/myproject/file/"), None);
    }

    #[test]
    fn test_parse_repo_unknown_action() {
        assert_eq!(parse_uri("ferret://repo/myproject/unknown"), None);
    }
    ...
    fn test_read_status_empty() {
        ...
        assert!(content.contains("ferret status: healthy"));
    ...
    fn test_read_status_with_segments() {
        ...
        let base = dir.path().join(".ferret_index/segments");
        std::fs::create_dir_all(&base).unwrap();

        let seg = Arc::new(
            ferret_indexer_core::SegmentWriter::new(&base, ferret_indexer_core::SegmentId(0))
                .build(vec![
                    ferret_indexer_core::InputFile {
                        path: "a.rs".to_string(),
    ...
    fn test_read_file_found() {
        ...
        let seg = Arc::new(
            ferret_indexer_core::SegmentWriter::new(&base, ferret_indexer_core::SegmentId(0))
                .build(vec![ferret_indexer_core::InputFile {
                    path: "src/main.rs".to_string(),
    ...
    fn test_read_resource_status() {
        let state = Arc::new(IndexState::new());
        let result = read_resource(&state, "ferret://status");
        assert!(result.is_ok());
        let result = result.unwrap();
        assert_eq!(result.contents.len(), 1);
        match &result.contents[0] {
            ResourceContents::TextResourceContents { text, uri, .. } => {
                assert_eq!(uri, "ferret://status");
                assert!(text.contains("ferret status: healthy"));
            }
    ...
    fn test_read_resource_file() {
        ...
        let result = read_resource(&state, "ferret://repo/myproject/file/hello.txt");
        assert!(result.is_ok());
        let result = result.unwrap();
        match &result.contents[0] {
    ...
    fn test_read_resource_tree() {
        let state = Arc::new(IndexState::new());
        let result = read_resource(&state, "ferret://repo/myproject/tree");
        assert!(result.is_ok());
        let result = result.unwrap();
        match &result.contents[0] {
            ResourceContents::TextResourceContents { text, .. } => {
    ...
    fn test_read_resource_repo_status() {
        let state = Arc::new(IndexState::new());
        let result = read_resource(&state, "ferret://repo/myproject/status");
        assert!(result.is_ok());
        let result = result.unwrap();
        match &result.contents[0] {
            ResourceContents::TextResourceContents { text, .. } => {
    ...
    fn test_list_resources() {
        ...
        assert_eq!(result.resources[0].uri, "ferret://status");
    }
