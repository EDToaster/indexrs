//! MCP server implementation for ferret.
//!
//! [`FerretServer`] exposes the code index to AI assistants via the Model
...
use ferret_indexer_core::index_state::IndexState;
use ferret_indexer_core::query::parse_query;
use ferret_indexer_core::search::SearchOptions;
...
#[tool_router]
impl FerretServer {
    /// Get ferret server version and basic status.
    #[tool(
        description = "Get ferret server version and basic status. Call this first to verify the server is running."
    )]
    fn ping(&self) -> String {
        format!("ferret MCP server v{}", env!("CARGO_PKG_VERSION"))
    }
    ...
    async fn search_code(
        ...
        if let Some(daemon) = &self.daemon {
            let req = ferret_indexer_daemon::DaemonRequest::QuerySearch {
                query: query_string,
    ...
    /// Get the current status of the ferret service.
    #[tool(
        name = "index_status",
    ...
    async fn index_status(
        ...
        if let Some(repo) = &params.repo {
            ...
            let mut output = String::new();
            output.push_str(&format!("ferret status: {status}\n"));
            output.push_str(&format!("Uptime: {uptime}\n\n"));
            output.push_str(&format!("Segments: {segment_count}\n"));
    ...
    async fn reindex(
        ...
        if let Some(daemon) = &self.daemon {
            match daemon
                .request(ferret_indexer_daemon::DaemonRequest::Reindex { compact: false })
                .await
            {
                Ok(result) => {
    ...
    async fn search_symbols(
        ...
        let kind_filter = match &params.kind {
            Some(k) => match ferret_indexer_core::SymbolKind::from_str_loose(k) {
                Some(sk) => Some(sk),
                None => {
                    return Ok(errors::invalid_parameter(
                        "kind",
        ...
        // Dispatch through daemon if available
        if let Some(daemon) = &self.daemon {
            let req = ferret_indexer_daemon::DaemonRequest::Symbols {
...
impl FerretServer {
    ...
    async fn search_symbols_direct(
        &self,
        query: &str,
        kind: Option<ferret_indexer_core::SymbolKind>,
        language: Option<ferret_indexer_core::Language>,
        path: Option<&str>,
        max_results: usize,
        offset: usize,
    ) -> Result<CallToolResult, ErrorData> {
    ...
    async fn search_symbols_direct(
        &self,
        _query: &str,
        _kind: Option<ferret_indexer_core::SymbolKind>,
        _language: Option<ferret_indexer_core::Language>,
        _path: Option<&str>,
        _max_results: usize,
        _offset: usize,
    ) -> Result<CallToolResult, ErrorData> {
    ...
    async fn search_code_direct(
        ...
        // Execute the search
        let result = match ferret_indexer_core::search_segments_with_query(
            &snapshot,
            &query,
            &search_options,
        ) {
...
fn segment_disk_size(segment: &ferret_indexer_core::segment::Segment) -> u64 {
...
mod tests {
    ...
    use ferret_indexer_core::types::{FileId, SegmentId};
    ...
    fn build_test_server(
        ...
        FerretServer::new(state, None, None)
    }
    ...
    fn build_test_segment(
        ...
        files: Vec<InputFile>,
    ) -> Arc<ferret_indexer_core::segment::Segment> {
    ...
    fn test_server_creation() {
        let state = Arc::new(IndexState::new());
        let server = FerretServer::new(state, None, None);
    ...
    fn test_server_creation_with_root() {
        ...
        let server = FerretServer::new(state, Some(root.clone()), None);
    ...
    fn test_server_info() {
        ...
        assert_eq!(info.server_info.name, "ferret");
    ...
    fn test_params_deserialize_full() {
        ...
        assert_eq!(params.repo.as_deref(), Some("ferret"));
    ...
    fn test_index_status_params_with_repo() {
        let json = serde_json::json!({"repo": "ferret"});
        let params: IndexStatusParams = serde_json::from_value(json).unwrap();
        assert_eq!(params.repo.as_deref(), Some("ferret"));
    }
    ...
    async fn test_index_status_empty_state() {
        ...
        assert!(text.contains("ferret status: empty"));
    ...
    async fn test_index_status_with_tombstones() {
        ...
        let mut ts = ferret_indexer_core::tombstone::TombstoneSet::new();
        ts.insert(FileId(0));
