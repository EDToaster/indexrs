...
pub fn run_status(repo_root: &Path) -> Result<(), indexrs_core::IndexError> {
    let manager = crate::repo::load_index(repo_root)?;
    let snapshot = manager.snapshot();

    if snapshot.is_empty() {
        println!("No index found. Run 'indexrs init' first.");
        return Ok(());
    }
...
fn print_segment_summary(
    ...
    if manager.should_compact() {
        println!("Recommendation: run 'indexrs reindex' to compact segments.");
    }
...
mod tests {
    use super::*;
    use indexrs_core::{InputFile, SegmentManager};
    use tempfile::tempdir;

    #[test]
    fn test_run_status_empty_index() {
        let dir = tempdir().unwrap();
        let index_dir = dir.path().join(".indexrs");
