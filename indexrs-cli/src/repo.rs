...
use indexrs_core::registry::{config_file_path, load_config_from};

/// Resolve a `--repo` value to a repo root path.
///
/// Resolution order:
/// 1. If a value is given, look it up as a repo name in `~/.config/indexrs/repos.toml`.
...
pub fn resolve_repo_with_config(
    ...
    match repo_arg {
        Some(value) => {
            ...
            if canonical.join(".indexrs").is_dir() || canonical.join(".git").exists() {
                Ok(canonical)
            } else {
...
mod tests {
    ...
    fn test_find_repo_root_explicit() {
        ...
        // The path must look like a valid repo (has .git or .indexrs).
    ...
    fn test_resolve_repo_by_name() {
        ...
        std::fs::create_dir_all(repo_path.join(".indexrs")).unwrap();

        let config_path = dir.path().join("repos.toml");
        let config = indexrs_core::registry::RepoConfig {
            repo: vec![indexrs_core::registry::RepoEntry {
                name: Some("myrepo".to_string()),
                path: repo_path.clone(),
            }],
        };
        indexrs_core::registry::save_config_to(&config_path, &config).unwrap();
    ...
    fn test_resolve_repo_by_path_fallback() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::create_dir(dir.path().join(".indexrs")).unwrap();
    ...
    fn test_resolve_repo_not_found() {
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join("repos.toml");
        let config = indexrs_core::registry::RepoConfig { repo: vec![] };
        indexrs_core::registry::save_config_to(&config_path, &config).unwrap();
