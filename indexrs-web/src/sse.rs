...
pub async fn search_stream(
    ...
    let request = indexrs_daemon::DaemonRequest::JsonSearch {
        query,
        page: 1,
    ...
    let event_stream = async_stream::stream! {
        loop {
            match indexrs_daemon::wire::read_response(&mut reader).await {
                Ok(indexrs_daemon::DaemonResponse::Json { payload }) => {
                    // Parse to determine event type.
                    match serde_json::from_str::<indexrs_daemon::JsonSearchFrame>(&payload) {
                        Ok(indexrs_daemon::JsonSearchFrame::Result { .. }) => {
                            yield Ok(Event::default().event("result").data(payload));
                        }
                        Ok(indexrs_daemon::JsonSearchFrame::Stats { .. }) => {
                ...
                Ok(indexrs_daemon::DaemonResponse::Done { .. }) => {
                    yield Ok(Event::default().event("done").data("{}"));
                    break;
                }
                Ok(indexrs_daemon::DaemonResponse::Error { message }) => {
