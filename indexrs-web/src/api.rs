...
pub struct SearchResponse {
    pub results: Vec<indexrs_core::search::FileMatch>,
...
pub struct RepoListEntry {
    pub name: String,
    pub path: PathBuf,
    pub status: Option<indexrs_daemon::StatusResponse>,
}
...
pub async fn add_repo(
    ...
    // Validate the path exists and has an .indexrs directory.
    if !path.is_dir() {
    ...
    if !path.join(".indexrs").is_dir() {
        return Err(ApiError::bad_request(format!(
            "path '{}' has no .indexrs directory; run indexrs index first",
            body.path
        )));
    }
    ...
    let mut config = indexrs_core::registry::load_config()
        .map_err(|e| ApiError::internal(format!("failed to load config: {e}")))?;

    if !indexrs_core::registry::add_repo(&mut config, path.clone(), body.name) {
        return Err(ApiError::bad_request(format!(
    ...
    indexrs_core::registry::save_config(&config)
...
pub async fn remove_repo(
    ...
    indexrs_core::registry::remove_repo(&mut config, &name);
